...
function stringToFr(str: string): Fr {
  const buf = Buffer.alloc(32);
  Buffer.from(str, 'utf8').copy(buf);
  return Fr.fromBufferReduce(buf.subarray(0, 32))
}
...
async function main() {
  ...
  for (const student of students) {
    ...
    // ];
    const firstname = stringToFr(student.firstName);
    const lastname = stringToFr(student.lastName);
    ...
    const message = [firstname, lastname, credit];
  ...
  const firstnameHex = stringToFr(student.firstName).toString();
  const lastnameHex = stringToFr(student.lastName).toString();
...
index = "${indexHex}"
lastname = "${lastnameHex}"
firstname = "${firstnameHex}"
root = "${rootHex}"
`.trim();
